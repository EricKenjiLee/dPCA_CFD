...
for m=1:length(sessData) %You can pass in listed savetags with sessData(m).savetags as the last argument instead of []
    currData = returnSessionFiringRatesPerCoherence(sessData(m),align,pre,post,'all',[],[450,1200]);
    firingRatesAverage = cat(1, firingRatesAverage, currData);

    currDataError = returnSessionFiringRatesPerCoherence(sessData(m),align,pre,post,'wrong',[],[450,1200]);
    firingRatesAverageError = cat(1, firingRatesAverageError, currDataError);

    allDataError = returnErrorFiringRates(sessData(m),align,pre,post); %THIS NEEDS TO BE FIXED TO MATCH NEW returnSessionFiringRatesPerCoherence FUNCTION
    firingRatesPooledError = cat(1, firingRatesPooledError, allDataError);

    currDataCorrect = returnSessionFiringRatesPerCoherence(sessData(m),align,pre,post,'correct',[],[450,1200]);
