function firingRatesAverage = returnSessionFiringRates(sessionData, alignV, preD, postD)
%
%
...
% CFDevents = CFDevents([CFDevents.correctness] == 1);
...
for jj = 1:length(cluId)
    ...
    for ii = 1:length(eventStruct)
        ...
        if ~isempty(spikeTime(currIdx))
            % RL
            if ismember(eventStruct(ii).cue, [180,214]) 
            ...
            % RR
            if ismember(eventStruct(ii).cue, [117 135]) 
            ...
            % GL
            if ismember(eventStruct(ii).cue, 225-[117 135]) 
            ...
            % GR
            if ismember(eventStruct(ii).cue, 225-[117 135]) & eventStruct(ii).chosenSide == 2
